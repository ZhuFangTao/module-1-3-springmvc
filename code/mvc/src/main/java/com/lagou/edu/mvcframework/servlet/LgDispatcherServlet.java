...
//        method.invoke() //
        ...
        //校验Security注解，判断url的用户参数是否有权限访问
        if (securityForbid(req, handler)) {
            resp.getWriter().write("Security forbid");
            return;
        }
    ...
    private boolean securityForbid(HttpServletRequest request, Handler handler) {
        ...
        Security security = handler.getController().getClass().getDeclaredAnnotation(Security.class);
